...
let currentStep    = 0;
const TOTAL_STEPS  = 9;
const STEP_NAMES   = ['Информация','Раса','Класс','Предыстория','Характеристики','Навыки','Заклинания','Снаряжение','Личность'];
...
function toast(msg, type = 'info', duration = 3000) {
  if (_logOpen) return;   // лог открыт — тосты не показываем
...
// ══════════════════════════════════════════════════════════════════
// CHAR LOG — система истории изменений персонажа (diff-based)
// ══════════════════════════════════════════════════════════════════

let _logOpen    = false;
let _logStem    = null;
let _logEntries = [];
let _logSnapshot = null;   // глубокая копия char при последнем сохранении

// Поля и их человекочитаемые названия + иконки
const LOG_FIELD_MAP = {
  hpCurrent:        { icon:'💔', label:'HP', fmt:(o,n)=> n>o ? `HP: <b>${o} → ${n}</b>` : `HP: <b>${o} → ${n}</b>`, iconFn:(o,n)=> n>o ? '💚' : '💔' },
  hpMax:            { icon:'❤️',  label:'Макс. HP', fmt:(o,n)=>`Макс. HP: <b>${o} → ${n}</b>` },
  hpTemp:           { icon:'🛡️',  label:'Врем. HP', fmt:(o,n)=>`Врем. HP: <b>${o} → ${n}</b>` },
  level:            { icon:'⬆️',  label:'Уровень',  fmt:(o,n)=>`Уровень: <b>${o} → ${n}</b>` },
  xp:               { icon:'✨',  label:'Опыт',     fmt:(o,n)=>`Опыт: <b>${o} → ${n}</b>` },
  ac:               { icon:'🛡️',  label:'КД',       fmt:(o,n)=>`КД: <b>${o} → ${n}</b>` },
  speed:            { icon:'💨',  label:'Скорость',  fmt:(o,n)=>`Скорость: <b>${o} → ${n}</b>` },
  initiative:       { icon:'⚡',  label:'Инициатива', fmt:(o,n)=>`Инициатива: <b>${o} → ${n}</b>` },
  name:             { icon:'✏️',  label:'Имя',       fmt:(o,n)=>`Имя: <b>${n}</b>` },
  raceName:         { icon:'🌍',  label:'Раса',      fmt:(o,n)=>`Раса: <b>${n}</b>` },
  className:        { icon:'⚔️',  label:'Класс',     fmt:(o,n)=>`Класс: <b>${n}</b>` },
  subclass:         { icon:'⚔️',  label:'Подкласс',  fmt:(o,n)=>`Подкласс: <b>${n}</b>` },
  alignment:        { icon:'⚖️',  label:'Мировоззрение', fmt:(o,n)=>`Мировоззрение: <b>${n}</b>` },
  backgroundName:   { icon:'📜',  label:'Предыстория', fmt:(o,n)=>`Предыстория: <b>${n}</b>` },
  exhaustion:       { icon:'😴',  label:'Истощение', fmt:(o,n)=>`Истощение: <b>${o} → ${n}</b>` },
  inspiration:      { icon:'🌟',  label:'Вдохновение', fmt:(o,n)=>`Вдохновение: <b>${n ? 'получено' : 'снято'}</b>` },
  proficiencyBonus: { icon:'📊',  label:'Бонус мастерства', fmt:(o,n)=>`Бонус мастерства: <b>+${o} → +${n}</b>` },
  hitDie:           { icon:'🎲',  label:'Кость хитов', fmt:(o,n)=>`Кость хитов: <b>к${n}</b>` },
  // характеристики
  'abilities.СИЛ':  { icon:'💪',  fmt:(o,n)=>`Сила: <b>${o} → ${n}</b>` },
  'abilities.ЛОВ':  { icon:'🏃',  fmt:(o,n)=>`Ловкость: <b>${o} → ${n}</b>` },
  'abilities.ТЕЛ':  { icon:'🏋️', fmt:(o,n)=>`Телосложение: <b>${o} → ${n}</b>` },
  'abilities.ИНТ':  { icon:'🧠',  fmt:(o,n)=>`Интеллект: <b>${o} → ${n}</b>` },
  'abilities.МДР':  { icon:'👁️',  fmt:(o,n)=>`Мудрость: <b>${o} → ${n}</b>` },
  'abilities.ХАР':  { icon:'🗣️',  fmt:(o,n)=>`Харизма: <b>${o} → ${n}</b>` },
  // текстовые поля
  traits:           { icon:'📝',  fmt:(o,n)=>`Черты характера изменены` },
  ideals:           { icon:'📝',  fmt:(o,n)=>`Идеалы изменены` },
  bonds:            { icon:'📝',  fmt:(o,n)=>`Привязанности изменены` },
  flaws:            { icon:'📝',  fmt:(o,n)=>`Слабости изменены` },
  backstory:        { icon:'📖',  fmt:(o,n)=>`История персонажа изменена` },
  appearance:       { icon:'🪞',  fmt:(o,n)=>`Внешность изменена` },
  age:              { icon:'🕰️',  fmt:(o,n)=>`Возраст: <b>${n}</b>` },
  height:           { icon:'📏',  fmt:(o,n)=>`Рост: <b>${n}</b>` },
  weight:           { icon:'⚖️',  fmt:(o,n)=>`Вес: <b>${n}</b>` },
  eyes:             { icon:'👁️',  fmt:(o,n)=>`Глаза: <b>${n}</b>` },
  skin:             { icon:'🖐️',  fmt:(o,n)=>`Кожа: <b>${n}</b>` },
  hair:             { icon:'💇',  fmt:(o,n)=>`Волосы: <b>${n}</b>` },
  allies:           { icon:'🤝',  fmt:(o,n)=>`Союзники/организации изменены` },
  attacksNotes:     { icon:'⚔️',  fmt:(o,n)=>`Заметки к атакам изменены` },
  inventoryNotes:   { icon:'🎒',  fmt:(o,n)=>`Заметки к инвентарю изменены` },
  ...
  spellAbility:     { icon:'✨',  fmt:(o,n)=>`Заклинательная характеристика: <b>${n||'—'}</b>` },
  halfProficiency:  { icon:'📊',  fmt:(o,n)=>`Мастер на все руки: <b>${n?'вкл':'выкл'}</b>` },
  remarkableAthlete:{ icon:'📊',  fmt:(o,n)=>`Выдающийся атлет: <b>${n?'вкл':'выкл'}</b>` },
  allowFirearms:    { icon:'🔫',  fmt:(o,n)=>`Огнестрел: <b>${n?'вкл':'выкл'}</b>` },
  subraceName:      { icon:'🌍',  fmt:(o,n)=>`Подраса: <b>${n||'—'}</b>` },
  carryBonus:       { icon:'⚖️',  fmt:(o,n)=>`Бонус грузоподъёмности: <b>${o||0} → ${n||0}</b>` },
  carryOverride:    { icon:'⚖️',  fmt:(o,n)=>`Грузоподъёмность вручную: <b>${n != null ? n+' фнт.' : 'сброшена'}</b>` },
  _spellAbilityOverride: { icon:'✨', fmt:(o,n)=>`Заклинательная характеристика: <b>${n||'авто'}</b>` },
  _spellAtkBonus:   { icon:'✨',  fmt:(o,n)=>`Бонус атаки заклинаниями: <b>${n > 0?'+':''}${n||0}</b>` },
  _spellAtkOverride:{ icon:'✨',  fmt:(o,n)=>`Атака заклинаниями вручную: <b>${n != null ? (n>0?'+':'')+n : 'сброшена'}</b>` },
  _spellSaveBonus:  { icon:'✨',  fmt:(o,n)=>`Бонус СЛ заклинаний: <b>${n > 0?'+':''}${n||0}</b>` },
  _spellSaveOverride:{ icon:'✨', fmt:(o,n)=>`СЛ заклинаний вручную: <b>${n != null ? n : 'сброшена'}</b>` },
  _spellPrepEnabled:{ icon:'✨',  fmt:(o,n)=>`Лимит подготовки: <b>${n?'включён':'выключен'}</b>` },
  _spellPrepBonus:  { icon:'✨',  fmt:(o,n)=>`Бонус подготовки заклинаний: <b>${n > 0?'+':''}${n||0}</b>` },
  _spellPrepOverride:{ icon:'✨', fmt:(o,n)=>`Лимит подготовки вручную: <b>${n != null ? n : 'сброшен'}</b>` },
  _initOverride:    { icon:'⚡',  fmt:(o,n)=>`Инициатива вручную: <b>${n != null ? fmtMod(n) : 'сброшена'}</b>` },
  hitDieOverride:   { icon:'🎲',  fmt:(o,n)=>`Кость хитов вручную: <b>${n||'сброшена'}</b>` },
  acFormula:        { icon:'🛡️',  fmt:(o,n)=>`Формула КД: <b>${n||'авто'}</b>` },
  size:             { icon:'📏',  fmt:(o,n)=>`Размер: <b>${n}</b>` },
  currency:         { icon:'💰',  fmt:(o,n)=> {
    const names = {gp:'золото', sp:'серебро', cp:'медь', ep:'электрум', pp:'платина'};
    const parts = [];
    for (const k of ['gp','sp','cp','ep','pp']) {
      if ((o[k]||0) !== (n[k]||0)) parts.push(`${names[k]}: ${o[k]||0}→<b>${n[k]||0}</b>`);
    }
    return parts.length ? `Монеты: ${parts.join(', ')}` : null;
  }},
};

// Массивы — отдельная логика (счёт элементов)
const LOG_ARRAY_MAP = {
  weapons:            { icon:'⚔️',  add:'Добавлена атака',     del:'Удалена атака',     name: w=>w.name },
  inventory:          { icon:'🎒',  add:'Добавлен предмет',    del:'Удалён предмет',    name: i=>i.name||i.id },
  spells:             { icon:'✨',  add:'Заклинание добавлено', del:'Заклинание убрано', name: s=>typeof s==='string'?s:(s.name||'') },
...
function _logDiffNoteBlocks(oldBlocks, newBlocks) {
  const oldMap = Object.fromEntries((oldBlocks||[]).map(b=>[b.id, b]));
  const newMap = Object.fromEntries((newBlocks||[]).map(b=>[b.id, b]));
  for (const id of Object.keys(newMap)) {
    if (!oldMap[id]) { charLogAdd('📝', `Заметка добавлена${newMap[id].title ? ': <b>'+newMap[id].title+'</b>' : ''}`); }
    else if (oldMap[id].title !== newMap[id].title) { charLogAdd('📝', `Заметка переименована: <b>${newMap[id].title||'—'}</b>`); }
    else if (oldMap[id].text !== newMap[id].text) { charLogAdd('📝', `Заметка изменена${newMap[id].title ? ': <b>'+newMap[id].title+'</b>' : ''}`); }
  }
  for (const id of Object.keys(oldMap)) {
    if (!newMap[id]) { charLogAdd('🗑️', `Заметка удалена${oldMap[id].title ? ': <b>'+oldMap[id].title+'</b>' : ''}`); }
  }
}

// Основной diff
function charLogDiff(oldChar, newChar) {
  if (!oldChar || !newChar) return;

  // Простые поля
  for (const [key, cfg] of Object.entries(LOG_FIELD_MAP)) {
    if (key.includes('.')) {
      // Вложенное поле типа abilities.СИЛ
      const [obj, prop] = key.split('.');
      const ov = (oldChar[obj]||{})[prop];
      const nv = (newChar[obj]||{})[prop];
      if (ov !== nv) {
        const msg = cfg.fmt(ov, nv);
        const icon = (cfg.iconFn ? cfg.iconFn(ov, nv) : null) || cfg.icon;
        if (msg) charLogAdd(icon, msg);
      }
    } else if (key === 'currency') {
      const ov = oldChar.currency || {};
      const nv = newChar.currency || {};
      const msg = cfg.fmt(ov, nv);
      const icon = (cfg.iconFn ? cfg.iconFn(ov, nv) : null) || cfg.icon;
      if (msg) charLogAdd(icon, msg);
    } else {
      const ov = oldChar[key];
      const nv = newChar[key];
  ...
  // Массивы
  for (const [key, cfg] of Object.entries(LOG_ARRAY_MAP)) {
    const oldArr = oldChar[key] || [];
    const newArr = newChar[key] || [];
    if (JSON.stringify(oldArr) === JSON.stringify(newArr)) continue;
    // Добавленные (есть в new, нет в old — по имени)
    const oldNames = new Set(oldArr.map(cfg.name).filter(Boolean));
    const newNames = new Set(newArr.map(cfg.name).filter(Boolean));
    for (const n of newNames) if (!oldNames.has(n)) charLogAdd(cfg.icon, `${cfg.add}: <b>${n}</b>`);
    for (const n of oldNames) if (!newNames.has(n)) charLogAdd(cfg.icon, `${cfg.del}: <b>${n}</b>`);
    // Если просто изменился существующий элемент без смены имени — общая запись
    if (oldArr.length === newArr.length) {
      const changed = oldArr.some((o,i) => JSON.stringify(o) !== JSON.stringify(newArr[i]));
      if (changed) charLogAdd(cfg.icon, `${cfg.add.replace('добавлено','изменено').replace('добавлен','изменён').replace('добавлена','изменена')}: проверьте лист`);
    }
  }
  ...
  const ne = newChar.skillExpertise || {};
  const allSkills = new Set([...Object.keys(oe), ...Object.keys(ne)]);
  for (const sk of allSkills) {
    const ov = oe[sk]||0, nv = ne[sk]||0;
    if (ov === nv) continue;
    const lvl = ['','владение','компетентность'];
    if (nv > ov) charLogAdd('🎯', `Навык получен: <b>${sk}</b> (${lvl[nv]||nv})`);
    else if (nv < ov && nv > 0) charLogAdd('🎯', `Навык понижен: <b>${sk}</b> (${lvl[nv]||nv})`);
    else charLogAdd('🎯', `Навык утерян: <b>${sk}</b>`);
  }

  // usedSpellSlots
  const os = oldChar.usedSpellSlots || {};
  const ns = newChar.usedSpellSlots || {};
  for (const lvl of [...new Set([...Object.keys(os),...Object.keys(ns)])]) {
    const ov = os[lvl]||0, nv = ns[lvl]||0;
    if (ov===nv) continue;
    if (nv > ov) charLogAdd('🔮', `Ячейка ${lvl} ур. потрачена`);
    else         charLogAdd('🔮', `Ячейка ${lvl} ур. восстановлена`);
  }

  // _spellOverrides — ручные правки заклинаний
  {
    const ov = oldChar._spellOverrides || {}, nv = newChar._spellOverrides || {};
    const keys = new Set([...Object.keys(ov), ...Object.keys(nv)]);
    for (const k of keys) {
      const os = JSON.stringify(ov[k]), ns = JSON.stringify(nv[k]);
      if (os === ns) continue;
      const spellName = newChar.spells?.[k] || oldChar.spells?.[k] || `заклинание #${k}`;
      if (!ov[k] && nv[k]) charLogAdd('✨', `Заклинание изменено вручную: <b>${spellName}</b>`);
      else if (ov[k] && !nv[k]) charLogAdd('✨', `Правки заклинания сброшены: <b>${spellName}</b>`);
      else charLogAdd('✨', `Заклинание изменено: <b>${nv[k]?.name || spellName}</b>`);
    }
  }

  // _spellNotes — заметки к заклинаниям
  {
    const ov = oldChar._spellNotes || {}, nv = newChar._spellNotes || {};
    const keys = new Set([...Object.keys(ov), ...Object.keys(nv)]);
    for (const k of keys) {
      if (ov[k] !== nv[k]) {
        const spellName = newChar.spells?.[k] || oldChar.spells?.[k] || `заклинание #${k}`;
        charLogAdd('✨', `Заметка к заклинанию изменена: <b>${spellName}</b>`);
      }
    }
  }

  // _spellAbilities — заклинательная характеристика на заклинание
  {
    const ov = oldChar._spellAbilities || {}, nv = newChar._spellAbilities || {};
    const keys = new Set([...Object.keys(ov), ...Object.keys(nv)]);
    for (const k of keys) {
      if (ov[k] !== nv[k]) {
        const spellName = newChar.spells?.[k] || oldChar.spells?.[k] || `заклинание #${k}`;
        charLogAdd('✨', `Хар-ка заклинания изменена: <b>${spellName}</b> → <b>${nv[k]||'авто'}</b>`);
      }
    }
  }

  // _skillBonus / _skillOverride — бонусы к навыкам
  for (const map of ['_skillBonus', '_skillOverride']) {
    const ov = oldChar[map] || {}, nv = newChar[map] || {};
    const keys = new Set([...Object.keys(ov), ...Object.keys(nv)]);
    for (const k of keys) {
      if (JSON.stringify(ov[k]) !== JSON.stringify(nv[k])) {
        const label = map === '_skillBonus' ? 'Бонус навыка' : 'Навык вручную';
        charLogAdd('🎯', `${label} <b>${k}</b>: ${ov[k]??'—'} → <b>${nv[k]??'—'}</b>`);
      }
    }
  }

  // _saveBonus / _saveOverride — бонусы к спасброскам
  for (const map of ['_saveBonus', '_saveOverride']) {
    const ov = oldChar[map] || {}, nv = newChar[map] || {};
    const keys = new Set([...Object.keys(ov), ...Object.keys(nv)]);
    for (const k of keys) {
      if (JSON.stringify(ov[k]) !== JSON.stringify(nv[k])) {
        const label = map === '_saveBonus' ? 'Бонус спасброска' : 'Спасбросок вручную';
        const FULL = {СИЛ:'Сила',ЛОВ:'Ловк.',ТЕЛ:'Тело',ИНТ:'Инт.',МДР:'Мудр.',ХАР:'Хар.'};
        charLogAdd('🎯', `${label} <b>${FULL[k]||k}</b>: ${ov[k]??'—'} → <b>${nv[k]??'—'}</b>`);
      }
    }
  }

  // deathSaves
  const od = JSON.stringify(oldChar.deathSaves||{});
  const nd = JSON.stringify(newChar.deathSaves||{});
  if (od !== nd) charLogAdd('💀', 'Спасброски от смерти изменены');
}

// Снимок состояния (глубокое копирование только нужных полей)
function _logTakeSnapshot(char) {
  if (!char) return null;
  try { return JSON.parse(JSON.stringify(char)); } catch { return null; }
}
...
// ── Добавить запись ─────────────────────────────────────────────
function charLogAdd(icon, text) {
  if (!_logStem) return;
  const now  = new Date();
  const time = now.toLocaleTimeString('ru-RU', {hour:'2-digit', minute:'2-digit'});
  const date = now.toLocaleDateString('ru-RU', {day:'2-digit', month:'2-digit', year:'2-digit'});
  const entry = { icon, text, time, date, ts: now.toISOString() };
  _logEntries.push(entry);
  fetch(`/api/log/${encodeURIComponent(_logStem)}`, {
    method: 'POST', headers: {'Content-Type':'application/json'},
    body: JSON.stringify(entry)
  }).catch(()=>{});
  if (_logOpen) _logAppendEntry(entry, true);
}

// ── Рендер всего списка ─────────────────────────────────────────
function _logRender() {
  const body = document.getElementById('char-log-body');
  if (!body) return;
  body.innerHTML = '';
  if (!_logEntries.length) {
    body.innerHTML = '<div style="color:var(--text3);text-align:center;padding:.8rem;font-size:.75rem">История пуста</div>';
    return;
  }
  let lastDate = null;
  _logEntries.forEach(e => {
    ...
    body.appendChild(_logMakeRow(e, false));
  });
  body.scrollTop = body.scrollHeight;
}

function _logAppendEntry(entry, isNew) {
  const body = document.getElementById('char-log-body');
  if (!body) return;
  const empty = body.querySelector('[data-empty]');
  if (empty) empty.remove();
  body.appendChild(_logMakeRow(entry, isNew));
  body.scrollTop = body.scrollHeight;
}

function _logMakeRow(e, isNew) {
  const row = document.createElement('div');
  row.className = 'log-entry' + (isNew ? ' log-new' : '');
  if (isNew) setTimeout(() => row.classList.remove('log-new'), 1400);
  row.innerHTML =
    `<span class="log-icon">${e.icon}</span>` +
    `<span class="log-text">${e.text}</span>` +
    `<span class="log-meta">${e.time}</span>`;
  row.addEventListener('contextmenu', ev => _logCtxShow(ev, row));
  return row;
}

// ── Открыть / закрыть / переключить ─────────────────────────────
function charLogToggle() { _logOpen ? charLogClose() : charLogOpen(); }

function charLogOpen() {
  _logOpen = true;
  document.getElementById('char-log-panel')?.classList.remove('hidden');
  _logRender();
}
function charLogClose() {
  _logOpen = false;
  document.getElementById('char-log-panel')?.classList.add('hidden');
}
async function charLogClear() {
  if (!_logStem) return;
  _showConfirmDialog(
    'Очистить всю историю изменений персонажа? Это действие нельзя отменить.',
    [
      { label: 'Очистить', action: async () => {
          _logEntries = [];
          await fetch(`/api/log/${encodeURIComponent(_logStem)}`, {method:'DELETE'}).catch(()=>{});
          _logRender();
      }},
...
let _logCtxTargetRow = null;

function _logRowToText(row) {
  const icon = row.querySelector('.log-icon')?.textContent || '';
  const text = row.querySelector('.log-text')?.textContent || '';
  const meta = row.querySelector('.log-meta')?.textContent || '';
  return `${icon} ${text}  ${meta}`.trim();
}

function _logCtxShow(e, row) {
  e.preventDefault();
  _logCtxTargetRow = row;
...
function _logCtxHide() {
  ...
  _logCtxTargetRow = null;
}

function logCtxCopyLine() {
  if (!_logCtxTargetRow) return;
  navigator.clipboard.writeText(_logRowToText(_logCtxTargetRow)).catch(() => {
    const ta = document.createElement('textarea');
    ta.value = _logRowToText(_logCtxTargetRow);
...
function logCtxCopyAll() {
  const all = _logEntries.map(e =>
    `${e.date} ${e.time}  ${e.icon} ${(e.text || '').replace(/<[^>]+>/g, '')}`
  ).join('\n');
...
document.addEventListener('click', e => {
  const menu = document.getElementById('log-ctx-menu');
  if (menu && !menu.contains(e.target)) _logCtxHide();
}, true);

// Ctrl+C — копировать всё содержимое лога если панель открыта
document.addEventListener('keydown', e => {
  if (!_logOpen) return;
  if (e.ctrlKey && e.key === 'c') {
    // Если есть выделение в браузере — не перехватываем, пусть копируется как обычно
    const sel = window.getSelection();
    if (sel && sel.toString().trim()) return;
    // Иначе — копируем весь лог
    e.preventDefault();
    logCtxCopyAll();
  }
});
...
// Приводит xp и level персонажа к взаимно согласованному состоянию:
// — если xp соответствует более высокому уровню, чем записан → поднимаем уровень
// — если xp меньше минимального для текущего уровня → поднимаем xp до минимума
function _normalizeXpLevel(char) {
  const level = Math.min(20, Math.max(1, parseInt(char.level) || 1));
  const xp    = Math.max(0, parseInt(char.xp) || 0);
  const levelFromXp = levelForXp(xp);
  const minXpForLevel = xpForLevel(level);

  if (levelFromXp > level) {
    // XP тянет на более высокий уровень — повышаем
    char.level = levelFromXp;
    char.proficiencyBonus = pbForLevel(levelFromXp);
  } else if (xp < minXpForLevel) {
    // XP меньше минимального для уровня — корректируем XP
    char.xp = minXpForLevel;
  }
}
...
// Изменили XP → пересчитать уровень
function cedOnXpInput() {
  ...
  lvlEl.value = lvl;
  _cedUpdateBar(xp, lvl);
}

// Изменили уровень → скорректировать XP
function cedOnLevelInput() {
  const xpEl  = document.getElementById('ced-xp');
  const lvlEl = document.getElementById('ced-level');
  if (!xpEl || !lvlEl) return;
  let lvl = Math.min(20, Math.max(1, parseInt(lvlEl.value) || 1));
  lvlEl.value = lvl;
  const minXp = xpForLevel(lvl);
  const curXp = parseInt(xpEl.value) || 0;
  if (curXp < minXp) xpEl.value = minXp;
  const xp = parseInt(xpEl.value) || 0;
  _cedUpdateBar(xp, lvl);
}
...
function saveCharEditDialog() {
  const _prevLevel = currentChar?.level;
  if (!currentChar) return;
...
function wizardNext() {
  ...
  // ── Шаг 7: снаряжение — если ничего не выбрано ──
  if (currentStep === 7) {
  ...
  // ── Шаг 8 (последний): Личность — создаём персонажа ──
...
function renderCurrentStep() {
  const fns = [syncStep0, renderStep1, renderStep2, renderStep3,
               renderStep4, renderStep5, renderStep6Spells, renderStep7, renderStep8];
  fns[currentStep]?.();
}
...
function validateStep() {
  ...
  if (currentStep === 1 && wiz.race) {
    const subs = (wiz.race.subraces||[]).filter(s => enabledSources.has(s.source));
    if (subs.length && !wiz.subrace) {
      toast('⚠️ Выберите подрасу', 'error');
      const sel = document.querySelector('#subrace-container select');
      if (sel) { sel.style.borderColor = '#e05555'; setTimeout(() => { sel.style.borderColor = ''; }, 1500); sel.scrollIntoView({ behavior: 'smooth', block: 'center' }); }
      return false;
    }
  }
  if (currentStep === 3 && !wiz.background) { toast('⚠️ Выберите предысторию','error'); return false; }
...
function syncStep0() {
  ...
  s('w-name', wiz.name); s('w-level', wiz.level);
}
function collectStep0() {
  const g = id => document.getElementById(id)?.value || '';
  wiz.name  = g('w-name').trim();
  wiz.level = parseInt(g('w-level')) || 1;
}
...
function syncStep8() {
  ...
  s('w-traits', wiz.traits); s('w-ideals', wiz.ideals);
  s('w-bonds', wiz.bonds); s('w-flaws', wiz.flaws); s('w-backstory', wiz.backstory);
}
function collectStep8() {
  ...
  wiz.traits = g('w-traits'); wiz.ideals = g('w-ideals');
  wiz.bonds = g('w-bonds'); wiz.flaws = g('w-flaws'); wiz.backstory = g('w-backstory');
}
function renderStep8() { syncStep8(); }
...
function renderRaceTraits() {
  const c = document.getElementById('race-traits-container');
  if (!c) return;
  const race = wiz.race;
  ...
  if (race.traits) {
    parts.push(`<div class="section-title" style="margin-top:.9rem">
      ${race.icon || ''} Расовые черты — ${race.name}
    </div>
    <div class="note-text" style="white-space:pre-line;line-height:1.6">${race.traits}</div>`);
  }

  if (sub && sub.traits) {
    parts.push(`<div class="section-title" style="margin-top:.7rem">
      ✦ Черты подрасы — ${sub.name}
    </div>
    <div class="note-text" style="white-space:pre-line;line-height:1.6">${sub.traits}</div>`);
  }

  c.innerHTML = parts.length
    ? `<div class="info-box" style="margin-top:.8rem">${parts.join('')}</div>`
    : '';
}
...
function updateAbilityDisplay() {
  ...
  ABILITY_KEYS.forEach(key => {
    ...
    const rollLocked = mode === 'roll' && wiz.rollLocked !== false;
    if (upBtn) upBtn.disabled = rollLocked || base>=15 || rem<nextCost;
    if (dnBtn) dnBtn.disabled = rollLocked || base<=8;
    ...
    if (manWr) {
      ...
      if (inp) {
        inp.value = base;
        // В режиме roll блокируем ручной ввод замочком
        const locked = mode === 'roll' && wiz.rollLocked !== false;
        inp.disabled = locked;
...
function renderRollSection() {
  ...
  const locked = wiz.rollLocked !== false; // default true
  const lockIcon = locked ? '🔒' : '🔓';
  const lockTitle = locked ? 'Ручной ввод заблокирован — нажмите чтобы разблокировать' : 'Ручной ввод разблокирован — нажмите чтобы заблокировать';
  c.innerHTML = `
    <div class="info-box" style="margin-top:.8rem">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.5rem">
        <b>Распределите кубики по характеристикам:</b>
        <button onclick="toggleRollLock()" title="${lockTitle}"
          style="background:none;border:1px solid var(--border);border-radius:6px;
                 padding:.2rem .55rem;cursor:pointer;font-size:1.1rem;color:var(--text2);
                 transition:border-color .15s"
          onmouseover="this.style.borderColor='var(--accent)'"
          onmouseout="this.style.borderColor='var(--border)'">${lockIcon}</button>
      </div>
      <div class="form-row">
        ${ABILITY_KEYS.map(k => {
          const cur = wiz.abilities[k];
          return `
          <div class="form-group">
            <label>${ABILITY_FULL[k]}</label>
            <select onchange="assignRoll('${k}', parseInt(this.value))" id="ab-roll-sel-${k}">
              ${pool.map(v=>`<option value="${v}" ${cur===v?'selected':''}>${v}</option>`).join('')}
            </select>
          </div>`;
        }).join('')}
      </div>
...
function toggleRollLock() {
  wiz.rollLocked = !wiz.rollLocked;
  // Обновить состояние ручных полей через updateAbilityDisplay
  renderRollSection();
  updateAbilityDisplay();
}
...
// Назначить значение из пула характеристике — с обменом если значение уже занято
function assignRoll(key, val) {
  const pool = wiz.rollValues?._pool || [];
  val = parseInt(val);

  // Найти другую характеристику у которой уже стоит это значение
  const otherKey = ABILITY_KEYS.find(k => k !== key && wiz.abilities[k] === val);
  if (otherKey) {
    // Поменять местами: другой получает старое значение текущей
    wiz.abilities[otherKey] = wiz.abilities[key];
  }
...
function toggleSpell(type, name, checked, max) {
  if (checked) charLogAdd('✨', `Заклинание добавлено: <b>${name}</b>`);
  else charLogAdd('✨', `Заклинание убрано: <b>${name}</b>`);
  const arr    = type==='cantrip' ? wiz.selectedCantrips : wiz.selectedSpells;
  const dispId = type==='cantrip' ? 'cantrip-count-display' : 'spell-count-display';
  if (checked) {
...
function createCharacter() {
  ...
  const char = {
    ...
    racialTraits:wiz.race?.traits||'', subraceTraits:wiz.subrace?.traits||'', classFeatures:wiz.cls?.features||'',
    subclassFeatures:(()=>{
...
async function saveCharacter(char) {
  ...
  if (wiz.portrait && wiz.portrait.startsWith('data:image')) {
    ...
    try {
      const pr = await fetch(`/api/portrait/${encodeURIComponent(portraitKey)}`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
...
function rteSetValue(taId, html) {
  ...
  if (ta)  ta.value = html;
  if (div) {
    // Если строка не содержит HTML-тегов — это plaintext, конвертируем \n в <br>
    const isPlain = !/<[a-z]/i.test(html);
    div.innerHTML = isPlain ? html.replace(/\n/g, '<br>') : html;
  }
}
...
function addNoteBlock() {
  charLogAdd('📝', 'Добавлен блок заметок');
  if (!currentChar) return;
...
function _noteBlockDel(id) {
  const _delNote = (currentChar?.noteBlocks||[]).find(b=>b.id===id);
  charLogAdd('🗑️', `Заметка удалена${_delNote?.title?' "'+_delNote.title+'"':''}`);
...
async function saveSheet() {
  ...
  const _ap = {...currentChar};
  if (currentCharId)        _ap._id = currentCharId;
  else if (currentFilename) _ap._filename = currentFilename;
  const _sr = await fetch('/api/characters',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(_ap)});
  if (_sr.status === 403) {
    const _sd2 = await _sr.json().catch(()=>({}));
    toast('❌ ' + (_sd2.message || 'Достигнут лимит персонажей'), 'error');
  }
...
async function loadCharList() {
  ...
  const sortDir = localStorage.getItem('charListSortDir') || 'desc';
  const sortedChars = [...chars].sort((a, b) => {
    let cmp = 0;
    if (sortKey === 'name')    cmp = (a.name||'').localeCompare(b.name||'', 'ru');
    if (sortKey === 'level')   cmp = (a.level||0) - (b.level||0);
    if (sortKey === 'updated') cmp = (a.updatedAt||a.updated_at||'') > (b.updatedAt||b.updated_at||'') ? 1 : -1;
    return sortDir === 'asc' ? cmp : -cmp;
  });

  const sortArrow = d => d === 'asc' ? ' ↑' : ' ↓';
  const sortBar = `<div style="display:flex;align-items:center;gap:.6rem;margin-bottom:.75rem;flex-wrap:wrap">
    ...
    ${[['updated','По дате'],['name','По имени'],['level','По уровню']].map(([k,l])=>`
      <button onclick="setCharListSort('${k}')" style="font-size:.72rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;padding:0;border:none;background:transparent;color:${sortKey===k?'#fff':'var(--text3)'};cursor:pointer;transition:color .15s">${l}${sortKey===k?sortArrow(sortDir):''}</button>
    `).join('')}
  </div>`;
...
async function createBlankChar() {
  ...
  // Подбираем уникальное имя: "Новый герой", "Новый герой 1", "Новый герой 2"...
  const BASE = 'Новый герой';
  let name = BASE;
  let n = 1;
  ...
  const ABILITY_KEYS_BLANK = ['СИЛ','ЛОВ','ТЕЛ','ИНТ','МДР','ХАР'];
  const abilities = {};
  ABILITY_KEYS_BLANK.forEach(k => { abilities[k] = 10; });

  const blank = {
    name,
    level: 1, xp: 0,
    race: '', raceName: '', subrace: '', subraceName: '',
    ...
    backstory: '', traits: '', ideals: '', bonds: '', flaws: '', allies: '',
    abilities,
    abilityBases: {...abilities},
    racialBonuses: {},
    proficiencyBonus: 2,
    hpMax: 1, hpCurrent: 1, hpTemp: 0, hitDie: 8,
    ac: 10, speed: 30, initiative: 0,
    flySpeed: 0, swimSpeed: 0, climbSpeed: 0, darkvision: 0,
    savingThrows: [],
    armorProf: '', weaponProf: [], toolProf: '', otherProf: '',
    languages: [],
    skillProficiencies: [], skillExpertise: {},
    attacks: [], spells: [], cantrips: [],
    spellAbility: '', spellDC: 0, spellAtk: 0,
    usedSpellSlots: {},
    features: [], resources: [],
    inventory: [], currency: { gp:0, sp:0, cp:0, ep:0, pp:0 },
    portrait: null,
    noteBlocks: [],
    size: 'Средний',
    allowFirearms: false,
...
function renderSheet(char) {
  ...
  if (profEl) {
    ...
    if (autoText !== null) {
      // В textarea (для сохранения) — plaintext без тегов
      const plainText = autoText.replace(/<br>/g, '\n').replace(/<[^>]+>/g, '');
      profEl.value = plainText;
...
function computeAbilitiesText(char) {
  ...
  // ── Черты подрасы ──
  if (char.subraceTraits) {
    lines.push(`▶ Черты подрасы (${char.subraceName || 'подраса'})`);
    lines.push(char.subraceTraits);
  }
...
function computeProfText(char) {
  ...
  const lines = [];
  if (char.languages?.length) lines.push(`<b>Языки:</b> ${char.languages.join(', ')}`);
  ...
  if (_armorVal) {
    ...
    lines.push(`<b>Доспехи${_restriction}:</b> ${_armorVal}`);
  }
  if (char.weaponProf) {
    const wp = Array.isArray(char.weaponProf) ? char.weaponProf.join(', ') : char.weaponProf;
    if (wp) lines.push(`<b>Оружие:</b> ${wp}`);
  }
  ...
  ['Инструменты', 'Музыкальные инструменты', 'Прочее'].forEach(cat => {
    if (grouped[cat]?.length) lines.push(`<b>${cat}:</b> ${grouped[cat].join(', ')}`);
  });
  Object.entries(grouped).forEach(([cat, items]) => {
    if (!['Инструменты', 'Музыкальные инструменты', 'Прочее'].includes(cat))
      lines.push(`<b>${cat}:</b> ${items.join(', ')}`);
  });
...
function applyWeaponEdit() {
  const _editingWpnName = currentChar?.attacks?.[_editingWeaponIdx]?.name || 'оружие';
  ...
  const pb = currentChar.proficiencyBonus || profBonus(currentChar.level||1);
  charLogAdd('⚔️', `Атака изменена: <b>${w.name || 'оружие'}</b>`);
  renderWeaponsSheet(currentChar, pb);
  saveSheet();
  closeWeaponEdit();
}
...
function _wDel(idx) {
  ...
  _confirmDel('Удалить атаку?', `Удалить «${name}»? Это действие нельзя отменить.`, () => {
    charLogAdd('🗑️', `Атака удалена: <b>${name}</b>`);
...
function renderSpellsSheet(char, pb) {
  if (!getSpellAbility(char)) {
    document.getElementById('s-spell-meta').innerHTML =
      ...
      '<button class="btn btn-secondary" style="font-size:.78rem;padding:.3rem .9rem;margin-bottom:.4rem" onclick="enableSpellcasting()">🔮 Сделать заклинателем</button>';
...
function _randomSpells() {
  ...
  // Добавляем заклинания подкласса из spells.subclasses[], которых нет в основном пуле
...
function _buildAndSave() {
  ...
  const char = {
    ...
    racialTraits:race?.traits||'', subraceTraits:wiz.subrace?.traits||'', classFeatures:wiz.cls?.features||'',
    armorRestriction: wiz.cls?.armorRestriction || '',
    subclassFeatures:(()=>{
      const sub=(wiz.cls?.subclasses||[]).find(s=>(s.name||s)===wiz.subclass);
      return (typeof sub==='object'&&sub.features)||'';
...
function applyHpMaxOverride() {
  const _prevHpMax = currentChar?.hpMax;
  if (!currentChar) return;
  ...
  updateHpDisplay();
  _syncDeathSavesVisibility();
  if (_prevHpMax !== undefined && _prevHpMax !== currentChar.hpMax)
    charLogAdd('❤️', `Макс. HP изменён: <b>${_prevHpMax} → ${currentChar.hpMax}</b>`);
  saveSheet();
}
...
function doShortRest() {
  ...
  charLogAdd('⚡', `Короткий отдых: <b>+${heal} HP</b> (${newHp}/${currentChar.hpMax||10})`);
  _logHpPrev = newHp;
  } else {
    toast('⚡ Короткий отдых завершён.', 'info');
    charLogAdd('⚡', 'Короткий отдых завершён');
  }
...
function doLongRest() {
  ...
  charLogAdd('🌙', `Долгий отдых: HP полностью восстановлены (<b>${currentChar.hpMax}</b>)`);
  _logHpPrev = currentChar.hpMax;
  closeRestDialog();
}
...
// ── Drag state ────────────────────────────────────────────────────────
const DP_SNAP_EDGE  = 70;   // px от края — примагничивание
const DP_DRAG_THRESH = 6;   // px до начала drag
let _dpSnap     = 'right';  // 'right'|'left'|'bottom'|'top'|'free'
let _dpDragging = false;

function _dpDie() {
  return parseInt(getComputedStyle(document.documentElement).getPropertyValue('--dp-die')) || 75;
}

// Возвращает высоту панели в текущем состоянии (свёрнута или развёрнута)
// Нижняя граница навбара в координатах viewport
function _dpNavBottom() {
  const nav = document.querySelector('.nav-bar');
  if (!nav) return 0;
  return nav.getBoundingClientRect().bottom / _dpZ();
}

// Реальная высота/ширина viewport с учётом CSS zoom
function _dpViewH() {
  const z = parseFloat(document.documentElement.style.zoom) || 1;
...
// Перевод координат мыши в зумированные пиксели
function _dpCx(e) { return (e.touches ? e.touches[0].clientX : e.clientX) / _dpZ(); }
function _dpCy(e) { return (e.touches ? e.touches[0].clientY : e.clientY) / _dpZ(); }

// Высота панели кубиков в текущем состоянии
function _dpPanelHeight() {
  const panel = document.getElementById('dice-panel');
  if (!panel) return _dpDie();
  if (_dicePanelOpen) {
    const h = panel.getBoundingClientRect().height;
    return h > 0 ? h : panel.offsetHeight || _dpDie();
  }
  return _dpDie();
}

// Зажимает top (верхняя граница якоря в viewport) в допустимых пределах
function _dpClampTop(desiredTop, panelH) {
  const minTop = _dpNavBottom();          // верх не выше низа навбара
  const maxTop = _dpViewH() - (panelH != null ? panelH : _dpPanelHeight());
  return Math.max(minTop, Math.min(maxTop, desiredTop));
}


function _dpApplySnap(snap, anchor) {
  anchor = anchor || document.querySelector('.dice-panel-anchor');
  if (!anchor) return;
  _dpSnap = snap;
  anchor.classList.remove('dp-snap-right','dp-snap-left','dp-snap-free','dp-snap-top');
  anchor.classList.add('dp-snap-' + snap);
  if (_dicePanelOpen) _closeDicePanel();

  if (snap === 'right' || snap === 'left') {
    // curY = позиция курсора (центр d20), top = верхняя граница якоря
    const curY = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--dp-snap-y')) || (_dpViewH() / 2);
    const die  = _dpDie();
    const desiredTop = curY - die / 2;  // курсор в центре d20
    const top = _dpClampTop(desiredTop, die);  // свёрнута = высота die
    const side = snap === 'right' ? 'right:0;left:auto' : 'left:0;right:auto';
    anchor.style.cssText = `position:fixed;${side};top:${top}px;bottom:auto;transform:none;width:var(--dp-w);height:auto;touch-action:none;user-select:none;z-index:900`;
  }
  // free — остаётся на месте
}

function _dpInitDrag() {
  const anchor = document.querySelector('.dice-panel-anchor');
  if (!anchor) return;

  // Начальный snap
  _dpSnap = 'right';
  anchor.classList.add('dp-snap-right');
  // Начальный top: центр экрана, с учётом ограничений
  {
    const die = _dpDie();
    const initTop = _dpClampTop(_dpViewH() / 2 - die / 2, die);
    anchor.style.cssText = `position:fixed;right:0;left:auto;top:${initTop}px;bottom:auto;transform:none;width:var(--dp-w);height:auto;touch-action:none;user-select:none;z-index:900`;
  }
  ...
  const onMove = e => {
    const cx = _dpCx(e);
    const cy = _dpCy(e);
    if (!moved && Math.hypot(cx - startX, cy - startY) < DP_DRAG_THRESH) return;

    if (!moved) {
      moved = true; _dpDragging = true;
      if (_dicePanelOpen) _closeDicePanel();
      // Сбрасываем состояние нижней панели при начале drag
      if (_dpLowerTimer)  { clearTimeout(_dpLowerTimer); _dpLowerTimer = null; }
      _dpSavedBottom = null;
      if (anchor.classList.contains('dp-lower')) anchor.classList.remove('dp-lower');
      anchor.style.transition = 'none';
      anchor.classList.remove('dp-snap-right','dp-snap-left','dp-snap-top');
      anchor.classList.add('dp-snap-free');
      _dpSnap = 'free';
      const half = die / 2;
      const NAV_H = _dpNavBottom();
      const H = _dpViewH();
      let nx = cx - half;
      let ny = cy - half;
      ny = Math.max(NAV_H, Math.min(H - die, ny));
      anchor.style.cssText = `position:fixed;left:${nx}px;top:${ny}px;right:auto;bottom:auto;width:${die}px;height:${die}px;touch-action:none;user-select:none;z-index:900`;
    }

    const W = _dpViewW(), H = _dpViewH();
    const NAV_H = _dpNavBottom();
    const half = die / 2;

    // Snap-зоны: только левый и правый край, ниже навбара
    const EDGE = DP_SNAP_EDGE;
    if (cx <= EDGE && cy > NAV_H) {
      // Передаём cy как центр курсора; _dpApplySnap переведёт в top
      document.documentElement.style.setProperty('--dp-snap-y', cy + 'px');
      _dpApplySnap('left', anchor);
    } else if (cx >= W - EDGE && cy > NAV_H) {
      document.documentElement.style.setProperty('--dp-snap-y', cy + 'px');
      _dpApplySnap('right', anchor);
    } else {
      // Свободно — круг центрирован под курсором, с ограничениями
      // При drag панель всегда свёрнута (закрываем в начале drag)
      let nx = cx - half;
      let ny = cy - half;
      nx = Math.max(0, Math.min(W - die, nx));
      ny = Math.max(NAV_H, Math.min(H - die, ny));
      if (_dpSnap !== 'free') {
        anchor.classList.remove('dp-snap-right','dp-snap-left','dp-snap-top');
        anchor.classList.add('dp-snap-free');
  ...
  const onUp = () => {
    ...
    if (moved) {
      if (_dpSnap !== 'free') _dpApplySnap(_dpSnap, anchor);
      // Блокируем клик после drag
      ...
      setTimeout(() => { _dpDragging = false; }, 120);
    } else {
      _dpDragging = false;
    }
  };
  ...
  anchor.addEventListener('mousedown', e => {
    ...
    if (e.target.closest('.dp-slot') && _dicePanelOpen) return;
    moved = false; _dpDragging = false;
    startX = e.clientX / _dpZ(); startY = e.clientY / _dpZ();
    document.addEventListener('mousemove', onMove);
    document.addEventListener('mouseup',   onUp);
    e.preventDefault();
  });
  ...
  anchor.addEventListener('touchstart', e => {
    moved = false; _dpDragging = false;
    startX = e.touches[0].clientX / _dpZ(); startY = e.touches[0].clientY / _dpZ();
    document.addEventListener('touchmove', onMove, {passive:false});
    document.addEventListener('touchend',  onUp);
  }, {passive:true});

  // При изменении размера окна — прижимаем панель в допустимые границы
  window.addEventListener('resize', () => {
    const a = document.querySelector('.dice-panel-anchor');
    if (!a) return;
    const die  = _dpDie();
    const ph   = _dicePanelOpen ? _dpOpenPanelHeight() : die;
    const navH = _dpNavBottom();
    const H    = _dpViewH();
    const W    = _dpViewW();

    if (_dpSnap === 'right' || _dpSnap === 'left') {
      // Якорь может быть в top- или bottom-позиционировании
      if (a.style.bottom && a.style.bottom !== 'auto') {
        // bottom-режим: пересчитываем bottom
        const curBottom = parseFloat(a.style.bottom) || 0;
        const maxBottom = H - die;
        a.style.bottom = Math.max(0, Math.min(maxBottom, curBottom)) + 'px';
      } else {
        const curTop = parseFloat(a.style.top) || 0;
        a.style.top = _dpClampTop(curTop, ph) + 'px';
      }
    } else if (_dpSnap === 'free') {
      const curLeft = parseFloat(a.style.left) || 0;
      const curTop  = parseFloat(a.style.top)  || 0;
      a.style.left = Math.max(0, Math.min(W - die, curLeft)) + 'px';
      a.style.top  = Math.max(navH, Math.min(H - die, curTop)) + 'px';
    }
  });
}
...
function _dpShiftAnchor() {
  // Вызывается только для snap right/left
  // В _openDicePanel позиция уже рассчитана с учётом направления
}
...
let _dpPreOpenTop  = null;  // top якоря перед открытием
...
function _closeDicePanel() {
  ...
  const anchor = document.querySelector('.dice-panel-anchor');
  if (anchor) {
    const wasLower = anchor.classList.contains('dp-lower');
    // Отменяем незавершённый таймер предыдущего закрытия
    if (_dpLowerTimer) { clearTimeout(_dpLowerTimer); _dpLowerTimer = null; }
    // Free: снимаем dp-open — CSS transition width 90→75px
    if (_dpSnap === 'free') {
      const w = anchor.offsetWidth + 'px';
      anchor.classList.remove('dp-open-horiz', 'dp-open', 'dp-reversed');
      anchor.style.width = w;
      void anchor.offsetWidth; // reflow
      anchor.style.width = '';
    } else {
      anchor.classList.remove('dp-open-horiz', 'dp-open', 'dp-reversed');
    }
...
function _diceOutsideClick(e) {
  const anchor = document.querySelector('.dice-panel-anchor');
  if (anchor && !anchor.contains(e.target)) _closeDicePanel();
}
...
function setCharListSort(key) {
  const prev    = localStorage.getItem('charListSort') || 'updated';
  const prevDir = localStorage.getItem('charListSortDir') || 'desc';
  if (prev === key) {
    localStorage.setItem('charListSortDir', prevDir === 'asc' ? 'desc' : 'asc');
  } else {
    localStorage.setItem('charListSort', key);
    localStorage.setItem('charListSortDir', key === 'name' ? 'asc' : 'desc');
  }
  loadCharList();
}
